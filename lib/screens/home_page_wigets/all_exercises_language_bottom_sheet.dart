...
class AllExercisesLanguageBottomSheet extends StatelessWidget {
  AllExercisesLanguageBottomSheet({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Container(
        ...
        child: Obx(() {
          final List<lang.Data> languages = vm.filteredLanguages;
          final selectedCode = vm.selectedLangCode.value;
          ...
          return Column(
            ...
            children: [
              ...
              /// 🌐 LANGUAGE LIST (API DRIVEN)
              ...languages.map((lang.Data langItem) {
                final isSelected = selectedCode == langItem.languageCode;

                IconData icon = Icons.language;
                if (langItem.languageName == "Hindi") {
                  icon = Icons.translate;
                } else if (langItem.languageName == "Spanish") {
                  icon = Icons.public;
                }

                return GestureDetector(
                  onTap: () async {
                    await vm.updateLanguage(
                      langItem.languageCode ?? "en",
                      langItem.id ?? 2,
                    );
                    Get.back();
                  },
                  child: AnimatedContainer(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            (langItem.languageName ?? "").split(' ').first,
                            style: TextStyle(
                              fontSize: 15,
