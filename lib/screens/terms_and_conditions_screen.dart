...
class _TermsAndConditionScreenState extends State<TermsAndConditionScreen> {
  ...
  void initState() {
    super.initState();
    _termsFuture = _loadTermsContent();
  }

  String _cachedTermsContent() {
    final cached = userStore.termsCondition.validate();
  ...
  Future<String> _loadTermsContent() async {
    try {
      final decoded = await handleResponse(
        await buildHttpResponse(TERMS_SERVICE_URL, method: HttpMethod.GET),
      );
      ...
      if (content.isNotEmpty) {
        await userStore.setTermsCondition(content, isInitialization: true);
        return content;
      }
    } catch (e, s) {
      debugPrint('Failed to load terms content => $e');
      debugPrint('$s');
    }

    return _cachedTermsContent();
  }
