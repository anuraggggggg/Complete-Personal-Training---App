...
class _ViewAllDietState extends State<ViewAllDiet> {
  ...
  Future<void> getDietData() async {
    ...
    await getDietApi(
      page: page,
      widget.isFeatured == true ? "yes" : "no",
      widget.isCategory,
      isAssign: widget.isAssign,
      categoryId: widget.mCategoryId,
      isFav: widget.isFav,
    ).then((value) {
      appStore.setLoading(false);
      numPage = value.pagination!.totalPages;
      isLastPage = false;
      if (page == 1) mDietList.clear();
      mDietList.addAll(value.data!);
      setState(() {});
    }).catchError((e) {
      isLastPage = true;
      appStore.setLoading(false);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // BottomNavigationBar remains but without banner ads
