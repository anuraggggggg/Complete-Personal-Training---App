...
class _ViewAllProductScreenState extends State<ViewAllProductScreen> {
  ...
  Future<void> getAllProductData() async {
    ...
    await getProductApi(
      ...
      productId: widget.id.validate(),
    ).then((value) {
      appStore.setLoading(false);
      numPage = value.pagination!.totalPages;
      isLastPage = false;
      if (page == 1) mProductList.clear();
      mProductList.addAll(value.data!);
      setState(() {});
    }).catchError((e) {
      isLastPage = true;
      appStore.setLoading(false);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget(
        widget.isCategory == true ? widget.title.toString() : languages.lblProductList,
        elevation: 0,
        context: context,
      ),
      body: Stack(
      ...
      // BottomNavigationBar kept but without banner ads
